...
void GPIO::GenerateVariablePWM(int period, int first_pulse, int last_pulse,
                               int num_periods) {
    int pulse_diff = last_pulse-first_pulse;
    int pulse_step = pulse_diff/num_periods;
    for(int i = 0; i < num_periods;i++){
        GeneratePWM(period,first_pulse+pulse_step*i,1);
    }
